...
public:
    ...
    interleaved_binary_fuse_filter(size_t bins_, size_t max_bin_elements)
    {
        this->bins = bins_;
        this->segment_length = 1L << (int)floor(log(max_bin_elements) / log(3.33) + 2.25);;
        // the current implementation hardcodes a 18-bit limit to
        ...
        if (ftype == 8)
        {
            bin_words = (this->bins + 7) >> 3; // = ceil(bins/8)
            technical_bins  = bin_words << 3; // = bin_words * 8
        }
        else
        {
            bin_words = (this->bins + 15) >> 4; // = ceil(bins/8)
            technical_bins  = bin_words << 4; // = bin_words * 16
        }

        data = sdsl::int_vector<>(bin_size_ * this->bins, 0, ftype);
    ...
    bool add_bin_elements(size_t bin, std::vector<size_t>& elements)
    {
        ...
        sdsl::int_vector<> t2hash = sdsl::int_vector(this->bin_size_,0,64);
        sdsl::int_vector<> alone_positions = sdsl::int_vector(bin_size_);
        sdsl::int_vector<> rev_order_i = sdsl::int_vector(elements.size(),0,64);
        sdsl::int_vector<> reverse_hi = sdsl::int_vector(elements.size(),0,8);
        int alone_position_nr = find_alone_positions(elements, t2count, t2hash, alone_positions, rev_order_i);
...
//                std::cout << alone_position_nr << std::endl;
        ...
        std::cout << "start filling the filter" << std::endl;
        fill_filter(rev_order_i, reverse_hi, bin);
...
//        uint64_t hash = XXH3_64bits_withSeed((char*) &value, sizeof(value), iff_ptr->seed);
        ...
        __uint128_t x = (__uint128_t)hash * (__uint128_t) segment_count_length;
        size_t h0 = (uint64_t)(x >> 64);
        size_t h1 = h0 + segment_length;
        size_t h2 = h1 + segment_length;
        uint64_t hh = hash;
...
template <typename FingerprintType>
template<std::integral value_t>
class interleaved_binary_fuse_filter<FingerprintType>::counting_agent_type<value_t>::counting_vector : public std::vector<value_t>
{
private:
    //!\brief The base type.
